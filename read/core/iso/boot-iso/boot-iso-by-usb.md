...
``` sh
wget -c https://sourceforge.net/projects/mx-linux/files/Final/Xfce/MX-25_Xfce_x64.iso
```

就會下載一個檔案「`MX-25_Xfce_x64.iso`」。
...
``` sh
sudo dd if=./MX-25_Xfce_x64.iso of=/dev/sdc bs=4M status=progress && sync
```

或是執行下面指令，就會寫入「USB隨身碟」。

``` sh
sudo dd if=./MX-25_Xfce_x64.iso of=/dev/sdc bs=4M status=progress conv=fsync oflag=direct
```
